...
export default function ReportFoundModal({ onClose, onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:3001/api/found-items/create", {
        method: "POST",
        body: submitData,
      });
      ...
      if (!response.ok) {
        alert(data.message || "Error submitting found item.");
        return;
      }
    ...
    <div className="w-full min-h-screen font-sans bg-white">
      ...
      <header className="w-full bg-yellow-400 shadow px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-2xl font-bold text-gray-800">Report Found Item</h1>
          <button
            onClick={onClose}
        ...
        <div className="relative z-10 w-full h-full flex items-center justify-center px-4 py-4">
          <div className="w-full max-w-6xl bg-white backdrop-blur-sm rounded-2xl shadow-2xl p-8">
            <div className="mb-6 text-center">
              <h2 className="text-3xl font-bold text-gray-800 mb-2">Report Found Item</h2>
              <p className="text-gray-600">Fill out the form below to report an item you found. Help return it to its owner!</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-6">
              {/* Item Name */}
              <div>
                <label className="block text-gray-700 text-lg font-medium mb-2">Item Name *</label>
                <input
                  type="text"
                  name="itemName"
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  <label className="block text-gray-700 text-lg font-medium mb-2">Location Found *</label>
                  <input
                    type="text"
                    name="location"
                ...
                <div>
                  <label className="block text-gray-700 text-lg font-medium mb-2">Date Found *</label>
                  <input
                    type="date"
                    name="dateFound"
              ...
              <div>
                ...
                <div className="border-4 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-yellow-400 transition-all duration-300 flex flex-col justify-center items-center bg-white/80">
                  <input type="file" accept="image/*" onChange={handleFileChange} className="hidden" id="imageUpload" required />
                  <label htmlFor="imageUpload" className="cursor-pointer flex flex-col items-center w-full">
                    ...
                    <p className="text-gray-500 text-sm mb-4">Upload a photo of the found item (PNG, JPG, JPEG up to 5MB) *</p>
      ...
      <footer className="w-full bg-yellow-400 py-6 px-5 grid grid-cols-2 md:grid-cols-5 gap-6 text-sm">
        ...
        <div>
          ...
          <ul className="space-y-2">
            <li><a href="#" className="block transition-all duration-300 hover:text-gray-800 hover:translate-x-1">Lost</a></li>
            <li><button onClick={onClose} className="block transition-all duration-300 hover:text-gray-800 hover:translate-x-1 bg-transparent border-none p-0 text-left">Report Lost</button></li>
            <li><a href="#" className="block transition-all duration-300 hover:text-gray-800 hover:translate-x-1">Found</a></li>
